...
public class PersonaliseValidation {
	...
	public PersonaliseValidation blankEmail(){
		...
		driver.findElement(submitButtonLocator).click();
		String actEmailValidMsg= driver.findElement(emailValidationMsg).getText();
		String expEmailValidMsg= props.getProperty("Personalise.validationmsg.email.text");
		if(actEmailValidMsg.equals(expEmailValidMsg)){
			System.out.println("Actual matches with expected, expected emial validation message is "+ expEmailValidMsg);
		}else{
			System.out.println("Actuall Message: "+actEmailValidMsg+" does not match with expected message: "+expEmailValidMsg);
		}
		return this;
